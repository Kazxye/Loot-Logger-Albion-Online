"""
EvNewEquipmentItem - Evento quando um item de equipamento aparece
Baseado em ev-new-equipment-item.js
"""
...
def handle(event: dict):
    """
    Processa evento EvNewEquipmentItem.
    
    Parâmetros:
    - [0] = objectId (int)
    - [1] = itemNumId (int)
    - [2] = quantity (int)
    - [5] = craftedBy (string) - DEVE existir para equipment
    - [6] = quality (int, opcional)
    """
    ...
    # IMPORTANTE: Equipment items DEVEM ter craftedBy
    crafted_by = params.get(5)
    if not isinstance(crafted_by, str):
        return
