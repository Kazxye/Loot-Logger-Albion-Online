...
from gui.activation_screen import ActivationManager
...
class LootLoggerApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        # Inicia com tela de ativacao
        try:
            self._show_activation()
        except Exception as e:
            import traceback
            print(f"[ERRO] Falha ao mostrar tela de ativacao: {e}")
            traceback.print_exc()
    ...
    def _show_activation(self):
        """Mostra tela de ativacao."""
        self.activation_manager = ActivationManager(
            self, self.theme,
            on_success=self._on_activation_success,
            on_cancel=self._on_activation_cancel
        )
        self.activation_manager.show()
    ...
    def _on_activation_cancel(self):
        """Usuario cancelou ativacao."""
        self.destroy()
        sys.exit(0)
        ...
        try:
            ...
            if has_update:
                messagebox.showinfo(
                    ...
                    f"Uma nova versao ({remote}) esta disponivel!\n\n"
                    f"Sua versao: {current}\n\n"
