...
def run_security_checks():
    ...
    try:
        from security import run_security_checks as security_check
        from security import protect_memory, get_security_monitor
        ...
        if not passed:
            print(f"[SECURITY] Ambiente não seguro detectado: {reason}")
            return False
        
        # Inicia monitor em background
        monitor = get_security_monitor()
        monitor.start(on_violation=lambda r: os._exit(1))
        
        return True
    except ImportError:
        # Módulo de segurança não disponível (desenvolvimento)
        return True
...
def main():
    ...
    if not run_security_checks():
        print("[ERRO] Falha nas verificações de segurança.")
        sys.exit(1)
